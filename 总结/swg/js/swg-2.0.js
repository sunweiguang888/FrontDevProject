...
(function(){
	...
	function SwgNodes(nodes) {
		...
		/*//为原型设置DOM操作方法。（每次实例化对象都会执行该过程，所以如果设置过一次，则不再设置）
		 ...
		 arguments.callee.prototype = SwgNodesPrototype;
		 this.__proto__ = SwgNodesPrototype;//直到IE 11才能访问__proto__属性，IE低版本无法访问，所以无法为当提前实例重新设置prototype
		 }*/
	}
	/*SwgNodes的DOM操作方法*/
	var SwgNodesPrototype = {
		flag: true,
		/**
		 * 绑定事件和处理方法
		...
		bind: function(eventType, handler){
			...
			this.each(function(){
				if (this.addEventListener) {//当为同一元素的同一事件添加处理方法时，会根据添加顺序依次执行
					...
					this.attachEvent(dom0EventType, function () {
						handler.call(ts);
					});
				} else {//一般不会执行，上面两种方法基本已经适用各种浏览器
					var oldHandler = this[dom0EventType];
					this[dom0EventType] = function() {
			...
			/*var addEventListener = function(eventType, handler) {
				var dom0EventType = "on" + eventType;
				if (this.addEventListener) {
					//当为同一元素的同一事件添加处理方法时，会根据添加顺序依次执行
					this.addEventListener(eventType, handler, false);
				} else if (this.attachEvent) {
					//IE 5 6 7 8，但执行顺序与添加顺序相反
					this.attachEvent(dom0EventType, (function (domNode) {
		...
		trigger: function(eventType){
			this.each(function(){
				/*if(this[eventType]){ 废弃 不正规的方法
					this[eventType].call(this);
				}else{
					this["on" + eventType].call(this);//兼容safari
				}*/
				try{
					//DOM
		...
		filterFromNodes: function(selector, nodes){
			if(!selector){
				selector = "*";
			}
			return swg(new Sizzle(selector, null, null, nodes));
		},
		filter: function(selector){
			...
			return this.filterFromNodes(selector, array);
		},
		/**
		 * 查找兄弟节点
		 * @param selector
		 ...
		 */
		parent: function(){
			return swg(this.nodes[0].parentNode);
		},
		/**
		 * 遍历节点
		 * @param handler
		 ...
		 */
		eq: function(index){
			return this[index] ? swg(this[index]) : swg();
		},
		/**
		 * 获取第一个查询结果
		 * @param index {number} 序号
	...
	(function(){
		...
		for(var i in events){
			(function(){
				var event = events[i];
				SwgNodesPrototype[event] = function(handler){
	...
	//绑定原型
	SwgNodesPrototype.constructor = SwgNodes;
	SwgNodes.prototype = SwgNodesPrototype;
	...
	var swg = function(param){
		if(param instanceof Function){//如果是函数，则绑定load事件
			new SwgNodes(window).bind("load", param);
		}else if(param instanceof String || typeof param == "string"){//如果是字符串，用过滤器进行过滤 || param instanceof String
			return new SwgNodes(Sizzle(param));
		}else if(typeof param == "object"){//如果是node
			return new SwgNodes(param);
		}else{
			return new SwgNodes();
		}
	};
